...
func main(){
	...
	//Connect to the backend server selector on 9000
	server,err = net.Dial("tcp", ":9000")
...
//direct the user to the page entered on the url
...
func home(w http.ResponseWriter, r *http.Request){
	...
	if logged{
		...
		//fmt.Print("Message to server: " + message)
		server.Write([]byte(message))
		readbuf := bufio.NewScanner(server)
		for readbuf.Scan(){
			serverResponse := readbuf.Text()
			...
			if serverResponse == "End"{
				break
			}else if len(serverResponse) != 0 && len(responseList) == 3{
...
func selectfollow(w http.ResponseWriter, r *http.Request){
	...
	for _,line := range notFollowedList{
		if line == cookie.Value || len(line) == 0 || line == "End"{
			continue
		}
...
func selectunfollow(w http.ResponseWriter, r *http.Request){
	...
	for _,line := range followlist{
		if len(line) == 0 || line == cookie.Value || line == "End"{
			continue
		}
		output := "<input type='checkbox' name='uname' value='" +
				  line + "'/>" + line + "</br>"
